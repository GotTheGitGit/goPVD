...
def Rank():
    ...
    top_column = []
    for row in rows:
        ranks = row.find_all('td')[0].text.ljust(5)
        ranking.append(ranks)
        name = row.find_all('td')[1].text
        if len(name) == 2:
            name = f'{name}　'
        names.append(name.ljust(6))
        win = row.find_all('td')[2].text.ljust(5)
        total_win.append(win)
        loss = row.find_all('td')[3].text.ljust(4)
        total_loss.append(loss)
        winrate = row.find_all('td')[4].text.ljust(7)
    ...
    title = ' '.join(top_column)
    reply_list.append(title)
    for g in range(99):
        data_list = [ranking[g], names[g], total_win[g],
                     total_loss[g], winrates[g], total_games[g]]
        reply_list.append(data_list)
