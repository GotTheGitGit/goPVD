...
def Rank():
    ...
    #print(soup.prettify())
    titles = soup.find('table', class_ = 'table table-bordered table-striped').thead.find_all('tr')
    rows = soup.find('table', class_ = 'table table-bordered table-striped').tbody.find_all('tr')
    ...
    first_line = "凡" + timeString + "年參與PlayGo所辦理之賽事三場以上之六段以上選手，均列入" + timeString + "年勝率排行榜。"
